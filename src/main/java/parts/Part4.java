...
public class Part4 {
    ...
    public void codec3(String text){
        ...
        List<Float> compressionValues = new ArrayList<>();
        ...
        while (Mdest <= maxWindowSize){
            ...
            while (Ment < Mdest){
                ...
                float compressionFactor = inputLen /  encodedTxt.length();

                outlen.add(encodedTxt.length());
                MentValues.add(Ment);
                MdestValues.add(Mdest);
                compressionValues.add(compressionFactor);
